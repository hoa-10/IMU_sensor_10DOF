...
Dataset được gom thành **10 nhóm (10 classes)** hoàn chỉnh:

### A. Nhóm Hoạt động Hàng ngày (8 Classes):
1. **Đi bộ** (`walking`)
2. **Chạy bộ** (`running`) — Chạy đều, chạy dưỡng sinh (jogging).
3. **Chạy gấp rút** (`sprinting`) — Chạy bứt tốc nhanh, dậm chân mạnh.
4. **Đứng yên** (`standing`)
5. **Ngồi** (`sitting`)
6. **Nằm** (`lying`)
7. **Cầu thang** (`stairs`)
...
10. **Khó thở** (`dyspnea`) — Bao gồm cả **Thở nhanh/thở dốc** và **Thở chậm/ngưng thở** được gom chung vào folder `dyspnea`.

---

## 📋 2. Bảng Phân bổ Thu thập Cho 1 Người (5 Lần x 1 Phút)

| STT | Tên Hoạt động | Label Key | Số lần (Hiệp) | Thời gian mỗi lần | Số dòng thu được / Người | Hướng dẫn thực hiện |
| :---: | :--- | :---: | :---: | :---: | :---: | :--- |
...
| **9** | **Khó thở** | `dyspnea` | **5 lần** | **1 phút** | 15.000 dòng | • Hiệp 1, 2, 3: Thở dồn dập, thở dốc $25 \sim 35$ nhịp/phút.<br>• Hiệp 4, 5: Thở rất chậm ($6 \sim 8$ nhịp/phút) hoặc nín thở $10\text{s}$. |
...
Mỗi activity sẽ lưu vào folder riêng hoặc đặt tên file chuẩn:
```text
sub01_walking_t01.csv   -> sub01_walking_t05.csv
sub01_dyspnea_t01.csv   -> sub01_dyspnea_t05.csv
